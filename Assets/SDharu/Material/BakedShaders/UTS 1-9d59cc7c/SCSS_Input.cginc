...
{
	float3 ambientLightDirection = Unity_SafeNormalize((unity_SHAr.xyz + unity_SHAg.xyz + unity_SHAb.xyz) * float4(1,0.1,1,0).xyz);

	if (float(0) == 2) // Flatten
	{
...
SCSS_RimLightInput initialiseRimParam()
{
	...
	rim.width = float(0.5);
	rim.power = float(0.5);
	rim.tint = float4(1,1,1,1).rgb;
	rim.alpha = float4(1,1,1,1).a;

	rim.invWidth = float(0.5);
	rim.invPower = float(0.5);
	rim.invTint = float4(1,1,1,1).rgb;
	rim.invAlpha = float4(1,1,1,1).a;
	return rim;
}
...
float2 AnimateTexcoords(float2 texcoord)
{
	...
	if (float(0))
	{
		float currentFrame = float(0) + frac(_Time[0] * float(10)) * float(4);

		float frame = floor(clamp(currentFrame, 0, float(4)));

		float2 offPerFrame = float2((1 / (float)float(2)), (1 / (float)float(2)));
		...
		float rowIndex;
		float mod = modf(frame / (float)float(2), rowIndex);
		currentSprite.y -= rowIndex * offPerFrame.y;
		currentSprite.x -= rowIndex * float(2) * offPerFrame.x;
...
float4 TexCoords(float4 uvPack0, float4 uvPack1)
{
	...
	texcoord.xy = float(0) ? 
		sharpSample(float4(0.0004882813,0.0004882813,2048,2048) * float4(1,1,0,0).xyxy, texcoord.xy) : texcoord.xy;

#if defined(_DETAIL) 
	float2 detailTexcoord = 
	float2(float4(uv0.x, uv1.x, uv2.x, uv3.x)[float(0)], 
		   float4(uv0.y, uv1.y, uv2.y, uv3.y)[float(0)]);
	texcoord.zw = TRANSFORM_TEX((detailTexcoord), _DetailAlbedoMap);
	texcoord.zw = float(0) ? 
		sharpSample(float4(1,1,1,1) * float4(1,1,0,0).xyxy, texcoord.zw) : texcoord.zw;
#else
...
float4 EmissionTexCoords(float4 uvPack0, float4 uvPack1)
{
	...
	texcoord.xy = _PixelSampleMode? 
		sharpSample(float4(1,1,1,1) * float4(1,1,0,0).xyxy, texcoord.xy) : texcoord.xy;
	...
	detailTexcoord = _PixelSampleMode? 
		sharpSample(float4(1,1,1,1) * float4(1,1,0,0).xyxy, detailTexcoord) : detailTexcoord;
...
{
    return UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, uv*float4(1,1,0,0).xy+float4(1,1,0,0).zw).rgb;
}
...
half ClippingMask(float2 uv)
{
	...
	#endif 
	return saturate(alpha + float(0));
}

half Alpha(float2 uv, float2 uv0)
{
	half alpha = float4(1,1,1,1).a;
	switch(float(0))
	{
...
void applyVanishing (inout float alpha) {
    ...
    float vanishing = saturate(lerpstep(float(0), float(0), closeDist));
    alpha = lerp(alpha, alpha * vanishing, float(0));
}
...
inline float getInventoryMask(float2 in_texcoord)
{
    ...
    uint itemID = floor((in_texcoord.x) / float(1));
...
float3 applyMaskedHSVToAlbedo(float3 albedo, float mask)
{
	...
	float3 warpedAlbedo = TransformHSV(albedo, float(0), float(1), float(1));
...
void applyDetail(float4 texcoords, inout SCSS_Input c)
{
	...
	c.albedo *= LerpWhiteTo(float4(1,1,1,1).rgb, tintMask);
	if (float(0)) c.tone[0].col *= LerpWhiteTo(float4(1,1,1,1).rgb, tintMask);
	if (float(0)) c.tone[1].col *= LerpWhiteTo(float4(1,1,1,1).rgb, tintMask);
    ...
    if (float(0)) c.tone[0].col = applyDetailToAlbedo(c.tone[0].col, detailAlbedo, mask);
	if (float(0))  c.tone[1].col = applyDetailToAlbedo(c.tone[1].col, detailAlbedo, mask);
#endif
}
...
void applyVertexColour(float4 color, float isOutline, inout SCSS_Input c)
{
	switch (float(2))
	{
		...
		if (float(0)) c.tone[0].col *= color.rgb; 
		if (float(0)) c.tone[1].col *= color.rgb; 
		break;
		case 1: 
		c.albedo = lerp(c.albedo, color.rgb, isOutline); 
		if (float(0)) c.tone[0].col = lerp(c.tone[0].col, color.rgb, isOutline); 
		if (float(0))  c.tone[1].col = lerp(c.tone[1].col, color.rgb, isOutline); 
		break;
	}
}
...
half4 SpecularGloss(float2 texcoords, float2 detailTexcoords, half mask)
{
    ...
    sg.a = float(0) == 1? UNITY_SAMPLE_TEX2D(_MainTex, texcoords.xy).a : sg.a;

    sg.rgb *= float4(0.03310474,0.03310474,0.03310474,1) * float4(0.03310474,0.03310474,0.03310474,1).a; // Use alpha as an overall multiplier
    sg.a *= float(0.5); // _GlossMapScale is what Standard uses for this
#else
    sg = float4(0.03310474,0.03310474,0.03310474,1);
    sg.a = float(0) == 1? UNITY_SAMPLE_TEX2D(_MainTex, texcoords.xy).a : sg.a;
#endif

#if defined(_DETAIL) 
		float4 sdm = UNITY_SAMPLE_TEX2D_SAMPLER(_SpecularDetailMask,_DetailAlbedoMap,detailTexcoords.xy);
		sg *= saturate(sdm + 1-(float(1)*mask));		
#endif
...
half4 EmissionDetail(float2 uv)
{
	...
	//if (float(0) == 0) // Pulse
	{
		uv += float4(0,0,0,0).xy * _Time.y;
		half4 ed = UNITY_SAMPLE_TEX2D_SAMPLER(_DetailEmissionMap, _DetailEmissionMap, uv);
		if (float4(0,0,0,0).z != 0)
		{
			float s = (sin(ed.r * float4(0,0,0,0).w + _Time.y * float4(0,0,0,0).z))+1;
			ed.rgb = s;
		}
		return ed;
	}
...
half4 EmissiveAudioLink(float2 maskUV, float2 sweepUV)
{
	...
	// sample the texture
	col.rgb += (float(1) >= 1) ? audioLinkGetLayer(weights.r, float(1), float(1), float(0)) * float4(1,0.09065466,0,0) : 0;
	col.rgb += (float(2) >= 1) ? audioLinkGetLayer(weights.g, float(1), float(2), float(0)) * float4(0,1,0.09065466,0) : 0;
	col.rgb += (float(3) >= 1) ? audioLinkGetLayer(weights.b, float(1), float(3), float(0)) * float4(0.08898151,0,1,0) : 0;
	col.rgb += (float(0) >= 1) ? audioLinkGetLayer(weights.a, float(1), float(0), float(0)) * float4(0.09065466,0.09065466,0.09065466,0) : 0;
	col.a = 1.0;
	col.rgb *= mask * float(1);
#endif
	return col;
}
...
SCSS_TonemapInput Tonemap1st (float2 uv)
{
	...
	tonemap.rgb = tonemap * float4(0.7254258,0.5787836,0.5341032,1);
	SCSS_TonemapInput t = (SCSS_TonemapInput)1;
	t.col = tonemap.rgb;
	t.bias = tonemap.a;
	t.offset = t.bias * float(0.246);
	t.width = float(0.172);
	return t;
}
SCSS_TonemapInput Tonemap2nd (float2 uv)
{
	...
	tonemap.rgb *= float4(0.8962694,0.6938719,0.8803295,1);
	SCSS_TonemapInput t = (SCSS_TonemapInput)1;
	t.col = tonemap.rgb;
	t.bias = tonemap.a;
	t.offset = t.bias * float(0.347);
	t.width = float(0.206);
	return t;
}
...
float ShadingGradeMap (float2 uv)
{
	...
	// Red to match UCTS
	return adjustShadeMap(tonemap.r, float(0));
}
#endif
...
{    
	#if defined(SCSS_OUTLINE)
	col = lerp(col, col * float4(0.2140411,0.2140411,0.2140411,1).rgb, is_outline);
    if (float(0) == 2) 
    {
        col = lerp(col, float4(0.2140411,0.2140411,0.2140411,1).rgb, is_outline);
    }
    return col;
    #else
    return col;
...
{    
	#if defined(SCSS_OUTLINE)
	alpha = lerp(alpha, alpha * float4(0.2140411,0.2140411,0.2140411,1).a, is_outline);
    if (float(0) == 2) 
    {
        alpha = lerp(alpha, float4(0.2140411,0.2140411,0.2140411,1).a, is_outline);
    }
    return alpha;
    #else
    return alpha;
...
void applyOutline(float is_outline, inout SCSS_Input c)
{
    ...
    if (float(0)) c.tone[0].col = applyOutline(c.tone[0].col, is_outline);
	if (float(0))  c.tone[1].col = applyOutline(c.tone[1].col, is_outline);
...
float3 applyNearShading(float3 color, float3 worldPos, bool isFrontFace)
{
    ...
    depth = max(0, depth/float(0.1));
    depth = saturate(depth);
    depth = pow(depth, abs(float(1.5)));

    if (float(0) == 1)
    {
    	float4 shadowColor = isFrontFace ? float4(0,0,0,1) : float4(0,0,0,1);
