...
float3 addEmissiveDetail(float3 emission, float2 emissionDetailUV, out float alpha)
{
	...
	emission = emissionDetail.rgb * emission * float4(0,0,0,1).rgb;
	...
	) : SV_Target
{
	...
	applyAlphaClip(material.alpha, float(0.5), i.pos.xy, float(0));
	...
	#if defined(_SPECULAR)
	{
		...
		if (float(0) == 1)
		{
			...
			if (float(0)) material.tone[0].col = material.tone[0].col * (material.oneMinusReflectivity); 
			if (float(0))  material.tone[1].col = material.tone[1].col * (material.oneMinusReflectivity); 
		}
	}
	#endif
	...
	{
		float NdotH = d.NdotH;
		float fresnelLightMaskBase = LerpOneTo(NdotH, float(0));
		float fresnelLightMask = 
			saturate(pow(saturate( fresnelLightMaskBase), float(1)));
		...
		float rimInv = sharpenLighting(rlPow4 * material.rim.invWidth * fresnelLightMaskInv,
			material.rim.invPower) * float(1) * material.rim.invAlpha;
		float3 rimInvCol = rimInv * material.rim.invTint;

		float3 rimFinal = rimCol + rimInvCol;

		float applyToAlbedo = (float(0) == 1) + (float(0) == 4);
		float applyToFinal = (float(0) == 2);
		float applyToLightBias = (float(0) == 3) + (float(0) == 4);
		// Lit
	...
	if (float(0) >= 1 && p.isOutline <= 0) 
	{
		half2 matcapUV;
		if (float(0) == 1) matcapUV = getMatcapUVsOriented(p.normal, p.view, float3(0, 1, 0));
		if (float(0) == 2) matcapUV = getMatcapUVsOriented(p.normal, p.view, bitangentDir.xyz);

		float4 _MatcapMask_var = MatcapMask(texcoords);
		material.albedo = applyMatcap(_Matcap1, matcapUV, material.albedo, float4(1,1,1,1), float(0), float(1) * _MatcapMask_var.r);
		material.albedo = applyMatcap(_Matcap2, matcapUV, material.albedo, float4(1,1,1,1), float(0), float(1) * _MatcapMask_var.g);
		material.albedo = applyMatcap(_Matcap3, matcapUV, material.albedo, float4(1,1,1,1), float(0), float(1) * _MatcapMask_var.b);
		material.albedo = applyMatcap(_Matcap4, matcapUV, material.albedo, float4(1,1,1,1), float(0), float(1) * _MatcapMask_var.a);
	}

	material.emission.rgb += float4(0,0,0,1).xyz * (pow(rlPow4, rcp(float4(0,0,0,1).w+FLT_EPS)));
