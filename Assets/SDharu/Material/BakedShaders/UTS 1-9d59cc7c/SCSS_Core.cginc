...
{
    float3 vSSLight = l.dir + normalDirection * float(1); // Distortion
    float3 vdotSS = pow(saturate(dot(viewDirection, -vSSLight)), float(1)) 
        * float(1); 
    
    return lerp(1, attenuation, float(any(_WorldSpaceLightPos0.xyz))) 
                * (vdotSS + float(0)) * abs(float(0)-thickness)
                * (l.color + indirectLight) * float4(1,1,1,1);
...
float3 sampleCrossToneLighting(inout float remappedLight, SCSS_TonemapInput tone0, SCSS_TonemapInput tone1, float3 albedo)
{
	...
	half factorBorder = saturate(simpleSharpen(remappedLight, tone0.width + float(0), tone0.offset));
	...
	if (float(0) == 0) 	tone1.col = tone1.col * tone0.col;
	// if (float(0) == 1) 	tone1.col = tone1.col; // Just here for completeness
	...
	if (float(0) == 0) 	final = final*albedo;
	// if (float(0) == 1) 	final = final; // Just here for completeness
	
	final = lerp(final, albedo, factorBorder*float4(1,0,0,1));
...
float getRemappedLight(half perceptualRoughness, SCSS_LightParam d)
{
	...
	float remappedLight = d.NdotL * LerpOneTo(diffuseShadowing, float(1));
...
float applyAttenuation(half NdotL, half attenuation)
{
	...
	half shadeVal = float(0.246) - float(0.172) * 0.5;
	...
	float3 indirectLighting, float3 directLighting, SCSS_LightParam d)
{
	...
	*/
	...
	directLighting	= lerp(indirectLighting, directLighting, float(0.8));
	...
	float attenuation, SCSS_LightParam d, SCSS_Light l, SCSS_ShadingParam p)
{
	if (float(0) == 4) {
		...
		spec = pow(spec, (smoothness)*40) * float(1);
		spec = sharpenLighting(frac(spec), float(0.02))+floor(spec);
    	spec = max(0.02,spec);
    	spec *= UNITY_PI * rcp(float(1));
...
{	
	...
	iL.color = GetSHMaxL1();
	iL.dir = Unity_SafeNormalize((unity_SHAr.xyz + unity_SHAg.xyz + unity_SHAb.xyz) * float4(1,0.1,1,0).xyz);
	...
	fL.dir = Unity_SafeNormalize(fL.dir + (unity_SHAr.xyz + unity_SHAg.xyz + unity_SHAb.xyz) * float4(1,0.1,1,0).xyz);
...
float3 SCSS_ApplyLighting(SCSS_Input c, SCSS_ShadingParam p)
{
    ...
    for (int num = 0; num < 4; num++) {
    	 if ((unity_LightColor[num].r + unity_LightColor[num].g + unity_LightColor[num].b + p.vertexLight[num]) != 0.0)
    	{
	...
	float glowModifier = smoothstep(float(1), float(0), dot(effectLightShadow, sRGB_Luminance));
	if (float(0)) c.emission *= glowModifier;
	#endif
